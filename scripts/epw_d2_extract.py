...
ap = argparse.ArgumentParser(); ap.add_argument("--root", default="/home/gregb26/links/projects/rrg-cotemich-ac/gregb26/graphene/qe/epw/24k-24q"); a = ap.parse_args()
R = a.root; E_D = -4.2389
...
modesG = (5, 6); iA1 = int(np.argmin(np.abs(om[iK] * 1e3 - 120.5))) + 1
SgG = Sg(iG, modesG); SgK = Sg(iK, (iA1,)); wG = om[iG, 4:6].mean(); wK = om[iK, iA1 - 1]
...
for name, iq0, modes_of in (("Gamma", iG, lambda iq: (5, 6)), ("K", iK, lambda iq: (strongest_mode(iq),))):
...
P = np.load("results/epw/phself_path_1200_dg0.02.npz", allow_pickle=True)
...
gG = P["gamma_epw"][i10, jG, 4:6].mean() * 1e-3; gK = P["gamma_epw"][i10, jK, 2] * 1e-3; wG2 = P["omega"][jG, 4:6].mean() * 1e-3; wK2 = P["omega"][jK, 2] * 1e-3   # RAW EPW gamma___ (route 2 tests the raw output)
...
np.savez("results/epw/d2_extract_24k24q.npz", d2_table=np.array([(k, v1, v2) for k, (v1, v2) in list(d2.items()) + list(LIT.items())], dtype=object), units="S_g eV^2; S_D eV^2/A^2; v eV.A; omega eV; gamma eV (HWHM)", E_D=E_D, v_F_epw=v_epw, v_F_dft=v_dft, A_c=A_c, D2_over_g2_per_eV=D2_over_g2_per_eV,
...
print("saved results/epw/d2_extract_24k24q.npz")
