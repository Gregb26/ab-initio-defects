...
LBL_G = r"Taux d'amortissement $\Gamma\,N_\text{cells}$ (meV)"
...
if maps:
    ...
    ax.set_xlabel(r"Rayon de coupure $R_\text{cut}$ (mailles)"); ax.set_ylabel(LBL_G); ax.set_xticks(rcs)
    ...
    if S in maps:
        ...
        fig, ax = plt.subplots(figsize=(3.6, 2.8)); im = ax.imshow(np.abs(D), cmap="Blues", vmin=0, vmax=vmax, origin="lower")
        ...
        for i in range(len(grids)):
            for j in range(len(etas)):
                ax.text(j, i, f"{Z[i,j]:.0f}\n({D[i,j]:+.1f}\\,\\%)", ha="center", va="center", fontsize=7, color=INK if abs(D[i, j]) < 0.6 * vmax else "white")
        ax.set_xlabel(r"Élargissement $\eta$ (eV)"); ax.set_ylabel(r"Grille $k$ de sortie")
        ax.set_title(rf"{lab(S)}, $R_\text{{cut}}$ = {RC} : médiane de $\Gamma N_\text{{cells}}$ (meV)", loc="left", fontsize=8)
        cb = fig.colorbar(im, ax=ax, shrink=0.85); cb.set_label(rf"Écart à ({GRID}$^2$, {ETA} eV) (\%)"); save(fig, "fig_plateau")

    # ---- fig_level2
    fig, ax = plt.subplots(figsize=(3.4, 2.7)); Ns = [int(S.split("x")[0]) for S in maps]; ys = [maps[S][(RC, GRID, ETA)][0] for S in maps]
    ...
    ax.set_title(rf"$R_\text{{cut}}$ = {RC}, grille {GRID}$^2$, $\eta$ = {ETA} eV ; zone grise : $N<N_\text{{min}}$", loc="left", fontsize=8); ax.legend(ncol=2); save(fig, "fig_level2")
...
if os.path.exists(a.locality):
    ...
    fig, axs = plt.subplots(2, 2, figsize=(6.5, 4.8), sharey=True, sharex=True); axs = axs.ravel()
...
if os.path.exists(a.resonance):
    ...
    fig, axs = plt.subplots(3, 2, figsize=(6.5, 7.2)); (a1, a2), (a3, a4), (a5, a6) = axs
    a1.plot(x, R["Gamma_T"] * 1e3, color=C_T, label=r"matrice $T$ (exacte)"); a1.plot(x, R["Gamma_Born"] * 1e3, color=C_BORN, label="approximation de Born (2$^\\text{e}$ ordre)")
    ...
    a3.set_ylabel("Densité d'états (états/eV/cellule)"); a3.set_title(r"$\delta\rho = \rho_\text{dis}-\rho_0$", loc="left"); a3.legend()
...
if os.path.exists(a.analysis):
    ...
    # fig_M_map : deux panneaux (π, π*), séquentielle une teinte, hexagone de la ZB, K marqué
    fig, axs = plt.subplots(1, 2, figsize=(6.5, 3.2), sharey=True)
    vmax = max(Z["map_Vpi"].max(), Z["map_Vpistar"].max())
    for ax, key, name, let in ((axs[0], "map_Vpi", r"$\pi$", "a"), (axs[1], "map_Vpistar", r"$\pi^*$", "b")):
        sc = ax.scatter(Z["map_kx"], Z["map_ky"], c=Z[key], cmap="Blues", vmin=0, vmax=vmax, s=14, marker="h", linewidths=0)
        ax.add_patch(Polygon(bz_vertices(Z["map_B"]), closed=True, fill=False, ec=MUTED, lw=0.8)); ax.plot(*Z["map_K"], "o", mfc="none", mec=C_BORN, mew=1.2, ms=7)
        ax.annotate("$K$", Z["map_K"], xytext=(5, 4), textcoords="offset points", color=C_BORN, fontsize=8)
        ax.set_aspect("equal"); ax.grid(False); ax.set_xlabel(r"$k'_x$ (\AA$^{-1}$)"); ax.set_title(rf"$n'$ = {name}$(k')$", loc="left"); panel(ax, let)
    axs[0].set_ylabel(r"$k'_y$ (\AA$^{-1}$)")
    cb = fig.colorbar(sc, ax=axs, shrink=0.9, pad=0.02); cb.set_label(r"$\tilde V = A_\text{cell}\,|M_{n'n}(k',K)|$ (eV\,\AA$^2$)")
    save(fig, "fig_M_map")
    ...
    ax.axhline(0, color=MUTED, lw=0.8); ax.set_xlabel(r"Distance au site / demi-largeur de la boîte (le long de $\mathbf a_1$)"); ax.set_ylabel(r"$V_\text{ed}^L$ (eV)")
    ...
    axs[1].set_ylabel(r"$\max|M|\cdot N_\text{cells}$ (eV)")
    axs[0].set_title("Convention cellule unitaire : intensif", loc="left", fontsize=8); axs[1].set_title(r"Convention super-cellule $\times N_\text{cells}$ : extensif", loc="left", fontsize=8)
