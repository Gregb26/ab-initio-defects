...
for i, S in enumerate(["5x5", "8x8", "9x9", "12x12"]):
    ...
    ax.semilogy(L[f"{S}_dense_dist"], L[f"{S}_dense_w"], "o", color=COL[S], ms=3.2, zorder=2, label=f"dense (zero-padding)\npz–pz (site) = {onv:.2f} eV")
    if f"{S}_coarse_w" in L:
        ax.semilogy(L[f"{S}_coarse_dist"], L[f"{S}_coarse_w"], "x", color=C_REF, ms=4, zorder=3, label=f"grille {lab(S)} (aliasée)\npz–pz (site) = {float(L[f'{S}_coarse_onsite_pzvac']):.2f} eV")
...
a_lat = float(V["9x9_a"]); axc.axvline(1.42, color=MUTED, lw=0.8, ls=":"); axc.axvline(3 * a_lat, color=MUTED, lw=0.8, ls="--")
axc.text(1.42, 0.97, "1$^\\mathrm{er}$ voisin", transform=axc.get_xaxis_transform(), ha="left", va="top", fontsize=8, color=MUTED)
axc.text(3 * a_lat, 0.97, r"$R_\mathrm{cut} = 3$ mailles", transform=axc.get_xaxis_transform(), ha="left", va="top", fontsize=8, color=MUTED)
